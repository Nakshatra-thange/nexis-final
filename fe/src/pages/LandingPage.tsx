...
const FloatingHearts = () => {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const left = (c / cols) * 100 + (Math.random() - 0.5) * 3
      const top = (r / rows) * 100 + (Math.random() - 0.5) * 3
...
const FeatureCard = ({ icon, text }: { icon: React.ReactNode; text: string }) => (
  <div className="bg-white/60 backdrop-blur-sm border border-bubblegum/40 rounded-3xl p-8 flex flex-col items-center gap-4 hover:border-cherry-soda/50 hover:glow-pink transition-all duration-300 cursor-pointer group">
    ...
    <p className="text-ruby-petals text-sm font-medium text-bubbly text-center">
      {text}
    </p>
  </div>
)
    ...
    <div className="min-h-screen bg-pink-mist relative overflow-hidden">
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-6 max-w-6xl mx-auto">
        ...
        <img
          src="/logo.png"
          alt="Smooth Logo"
          className="h-[6rem] w-auto object-contain select-none"
        />
        ...
        >
          {connected ? 'Enter Smooth' : 'Launch App'}
        </button>
      </nav>
      ...
      <main className="relative z-10 flex flex-col items-center text-center px-6 pt-16 pb-12 max-w-3xl mx-auto">
        ...
        <div className="space-y-3 mb-12">
          <MagneticLine delay={0.1}>Just say “Abra Sol Dabra.”</MagneticLine>
        ...
        {/* Feature Cards */}
        <div
          className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-20 w-full animate-fade-in"
        ...
        <div className="mt-20 animate-fade-in" style={{ animationDelay: '0.4s' }}>
          <p
            className="text-lg md:text-xl text-ruby-petals font-bold text-bubbly tracking-wide"
      ...
      <footer className="relative z-10 text-center py-8 border-t border-bubblegum/30">
        <p className="text-xs text-ruby-petals/40 text-bubbly">© 2026 Private</p>
      </footer>
    </div>
  )
}
